...
def load_referee_history(conn):
    ...
    try:
        ...
        ref = pd.DataFrame(columns=required)
    ...
    if ref.empty:
        print(
            "    AVISO: sin histórico de árbitros en Phase 1. "
            "Tarjetas se entrenará con equipos + faltas + tendencia de liga."
        )
        return ref[required].copy()
...
def build_side_rows(match_df, ref_hist):
    ...
    for _, r in match_df.iterrows():
        ...
        common = {
            ...
            "referee": (
                str(r["referee"]).strip()
                if pd.notna(r["referee"]) and str(r["referee"]).strip()
                else "UNKNOWN"
            ),
...
def main():
    print()
    print("=" * 76)
    print("LALIGA EDGE PRO - CARDS MODEL V2")
    print("=" * 76)
    print(
