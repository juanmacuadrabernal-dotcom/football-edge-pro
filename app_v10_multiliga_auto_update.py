...
def run_laliga_full_updater():
    updater = BASE_DIR / "update_laliga_all_v1.py"

    if not updater.exists():
        return False, (
            "Falta update_laliga_all_v1.py en la carpeta del proyecto."
        )
...
with st.sidebar:
    ...
    if league_key == "laliga":
        if st.button(
            ...
            help=(
                "Actualiza resultados, estadísticas, árbitros, features, "
                "reentrena los modelos finales y descarga la próxima jornada."
            ),
        ):
            with st.spinner(
                "Actualizando toda LaLiga. Puede tardar varios minutos..."
            ):
                ok, output = run_laliga_full_updater()

            if ok:
                st.session_state["laliga_update_message"] = (
                    "✅ ACTUALIZACIÓN COMPLETA: datos, árbitros, modelos y jornada al día."
                )
