...
def make_calibrator():
    # Platt scaling: estable con muestras relativamente pequeñas.
...
def walk_forward_calibration(pred, summary):
    ...
    for market in MARKETS:
        ...
        for test_season in seasons:
            ...
            # La primera temporada OOS no tiene OOS previo
            # con el que calibrar.
...
def verdict_table(results):
    ...
    for market in MARKETS:
        ...
        avg_base_brier = float(
            g["baseline_brier"].dropna().mean()
        )

        # Criterio prudente:
        # - mejor que baseline en todos los folds calibrados
        # - AUC mediana >= 0.53
        approved = (
            folds >= 2
...
def main():
    print()
    print("=" * 72)
    print("ELITESERIEN EDGE PRO - CALIBRACION GOLES V2")
    print("=" * 72)
    print(
        "Calibración walk-forward con Platt scaling."
    )
    print()
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        metadata = fit_final_calibrators(
            pred
        )

    verdict = verdict_table(results)
    ...
    print("=" * 72)
    print(
        "CANDIDATO no significa rentable."
    )
    print(
