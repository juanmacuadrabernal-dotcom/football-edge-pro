...
st.set_page_config(
    page_title="Football Edge Pro v15.2.1",
    page_icon="⚽",
    layout="wide",
...
def _fotmob_query_name(team):
    norm = _crest_norm(team)
    return FOTMOB_QUERY_ALIASES.get(norm, str(team))


def _fotmob_candidates(payload):
    found = []
    ...
    def walk(obj):
        if isinstance(obj, dict):
            ...
            if isinstance(candidate, dict):
                if candidate.get("id") is not None and candidate.get("name"):
            ...
            if (
                ...
                and obj.get("name")
                and (
                    obj.get("leagueId") is not None
                    or obj.get("leagueName") is not None
                    or obj.get("type") == "team"
                )
            ):
                found.append(obj)
...
def fotmob_team_id(team):
    """
    Sólo se usa para LaLiga.
    Si FotMob no responde o la coincidencia es dudosa,
    la app mantiene las iniciales.
    """
    query = _fotmob_query_name(team)

    try:
        response = requests.get(
            "https://apigw.fotmob.com/searchapi/suggest",
            params={"term": query, "hits": 12, "lang": "en"},
            timeout=8,
        )
    ...
    target = _crest_norm(query)
    best_id = None
    best_score = -1.0

    for item in _fotmob_candidates(payload):
        name = str(item.get("name") or "")
        if not name:
            continue

        candidate = _crest_norm(name)
        ...
        try:
            if int(item.get("leagueId")) == 87:
                score += 0.22
        except Exception:
            league_name = _crest_norm(item.get("leagueName") or "")
...
def sidebar():
    with st.sidebar:
        ...
        st.markdown("<br><div style='font-size:10px;color:#74899e'>Football Edge Pro v15.2.1<br>Analistas, no apostadores.</div>",unsafe_allow_html=True)
