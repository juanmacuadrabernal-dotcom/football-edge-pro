...
from football_odds_theoddsapi import populate_match_odds, get_api_key, DEFAULT_BOOKMAKERS
...
def market_page(match,league,cat,min_ev):
    ...
    if cat=="sot":
        st.info("🎯 SOT: mantenemos el modelo, pero The Odds API no documenta cuotas de SOT total/equipo. Aquí queda respaldo manual para no mezclarlo con props de jugadores.")
    ...
    if get_api_key():
        if auto_status.get("ok"):
            ...
            st.caption("📡 Cuotas PREPARTIDO · The Odds API · " + " / ".join(books()[:3]) + (f" · créditos restantes: {rem}" if rem else ""))
        elif not auto_status.get("locked"):
            st.caption("📡 " + str(auto_status.get("message", "Sin cuotas automáticas para este mercado.")))
    else:
        st.warning("Falta THE_ODDS_API_KEY. La app no puede descargar cuotas automáticas todavía.")
...
def scan_page(match,league,min_ev):
    ...
    if get_api_key():
        if auto_status.get("ok"):
            st.caption("📡 Cuotas automáticas PREPARTIDO · The Odds API · " + " / ".join(books()[:3]))
        elif not auto_status.get("locked"):
            st.caption("📡 " + str(auto_status.get("message", "Sin cuotas automáticas para este partido.")))
    rows=[analyzed(r,min_ev) for r in market_rows(match,league) if r["cat"]!="1x2"]
    quoted=[r for r in rows if r["ev"] is not None]
    if not quoted:
        st.info("The Odds API no ha devuelto cuota para esas líneas en nuestras 3 casas todavía. Si ocurre solo en SOT es normal: ese mercado total/equipo no está documentado por el proveedor.")
...
def settings(league):
    ...
    if get_api_key():
        st.success("📡 The Odds API conectada · cuotas automáticas PREPARTIDO activas")
    else:
        st.error("📡 The Odds API sin configurar · añade THE_ODDS_API_KEY")
