...
def get_json(url, params):
    ...
    raise RuntimeError(str(last_error) if last_error else "Error HTTP")
...
def finished_matches_for_season(start_year):
    best = []
    best_label = None
    best_payload = None
...
def fetch_match_detail(season, match_id):
    ...
    for url in DETAIL_URLS:
        try:
            payload = get_json(url, {"matchId": int(match_id)})
            path.write_text(
    ...
    raise RuntimeError(f"matchId={match_id}: {last_error}")
...
def find_stat_pair(payload, aliases):
    aliases = {normalize_stat_key(x) for x in aliases}
    matches = []
            ...
            if key in aliases:
                for value_key in ("stats", "values", "value"):
                    if value_key in node:
                        ...
                        if not (pd.isna(h) and pd.isna(a)):
                            matches.append((h, a))
...
def header_teams(payload):
    ...
    return (
        general.get("homeTeam", {}) or {},
        general.get("awayTeam", {}) or {},
    )
...
def parse_match(season, fixture, payload):
    ...
    home_team = header_home.get("name") or fixture_home.get("name")
    away_team = header_away.get("name") or fixture_away.get("name")

    ref = find_referee(payload)

    hy, ay = find_stat_pair(
        payload,
        ["yellow cards", "yellowcards", "amarillas", "tarjetas amarillas"],
    )
    hr, ar = find_stat_pair(
        payload,
        ["red cards", "redcards", "rojas", "tarjetas rojas"],
    )
    hf, af = find_stat_pair(
        payload,
        ["fouls committed", "fouls", "foulscommitted", "faltas", "faltas cometidas"],
    )
    ...
    return {
        "match_id": match_id,
        "season": int(season),
        "utc_time": ko.isoformat() if ko is not None else None,
        "home_team": home_team,
        "away_team": away_team,
        "referee": ref,
        "home_yellow": hy,
        "away_yellow": ay,
        "total_yellow": hy + ay if pd.notna(hy) and pd.notna(ay) else np.nan,
        "home_red": hr,
        "away_red": ar,
        "total_red": hr + ar if pd.notna(hr) and pd.notna(ar) else np.nan,
        "home_fouls": hf,
        "away_fouls": af,
        "total_fouls": hf + af if pd.notna(hf) and pd.notna(af) else np.nan,
    }
...
def build_summary(history):
    ...
    valid["utc_time"] = pd.to_datetime(
        valid["utc_time"], utc=True, errors="coerce"
    )

    league_avg = float(
        pd.to_numeric(valid["total_yellow"], errors="coerce").mean()
    )
    ...
    for referee, g in valid.groupby("referee"):
        ...
        total_y = pd.to_numeric(g["total_yellow"], errors="coerce")
        ...
        rows.append({
            ...
            "avg_home_yellow": float(
                pd.to_numeric(g["home_yellow"], errors="coerce").mean()
            ),
            "avg_away_yellow": float(
                pd.to_numeric(g["away_yellow"], errors="coerce").mean()
            ),
            "avg_red": float(
                pd.to_numeric(g["total_red"], errors="coerce").mean()
            ),
            "avg_fouls": float(
                pd.to_numeric(g["total_fouls"], errors="coerce").mean()
            ),
            "last10_yellow": float(total_y.tail(10).mean()),
            "last20_yellow": float(total_y.tail(20).mean()),
...
def process_one(season, fixture):
    mid = int(fixture["id"])
    payload, downloaded = fetch_match_detail(season, mid)
    row = parse_match(season, fixture, payload)
...
def main():
    print()
    print("=" * 90)
    print("LALIGA EDGE PRO - ARBITROS FAST V2")
    print("=" * 90)
    print(f"Hilos paralelos: {MAX_WORKERS}")
    print("Usa EXACTAMENTE la misma caché de la versión anterior.")
    print("Puedes parar con Ctrl+C sin perder las descargas ya cacheadas.")
    print()

    if not DB_PATH.exists():
        raise SystemExit(
            f"No existe {DB_PATH}. Ejecuta desde la carpeta principal del proyecto."
        )
    ...
    for season in SEASONS:
        ...
        season_label, fixtures = finished_matches_for_season(season)
        print(
            f"FotMob season={season_label} | finalizados={len(fixtures)}"
        )
        ...
        try:
            with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                future_map = {
                    executor.submit(process_one, season, fixture): fixture
                ...
                for future in as_completed(future_map):
                    ...
                    if completed % 10 == 0 or completed == len(fixtures):
                        refs = sum(
                            1 for r in (all_rows + season_rows)
                            if r.get("referee")
                        )

                        print(
                            f"  [{completed:3d}/{len(fixtures)}] "
                            f"refs={refs} | "
                            f"nuevas={total_new} | "
                            f"cache={total_cache} | "
            ...
            print()
            print("Ctrl+C detectado. Guardando lo procesado antes de salir...")
            ...
            print("Guardado. Puedes volver a ejecutar este mismo script.")
            return

        all_rows.extend(season_rows)

        history, summary = save_all(all_rows)

        print(
            f"Checkpoint guardado -> "
            f"{len(history)} partidos | "
            f"{history['referee'].notna().sum()} con árbitro"
        )
        print()
    ...
    print("=" * 90)
    print(f"Partidos guardados: {len(history)}")
    print(
    ...
    print(f"Leídos desde caché: {total_cache}")
