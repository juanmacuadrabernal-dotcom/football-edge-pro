...
GOALS_MODEL = MODELS_DIR / "laliga_goals_v4_team_strength_poisson.joblib"
CORNERS_MODEL = MODELS_DIR / "laliga_corners_v1_poisson.joblib"
CARDS_MODEL = MODELS_DIR / "laliga_cards_v2_side_poisson.joblib"
SHOTS_MODEL = MODELS_DIR / "laliga_shots_on_target_v1_poisson.joblib"
...
class PredictionEngineLaLiga:
    def __init__(self):
        if not DB_PATH.exists():
            raise FileNotFoundError(f"No existe {DB_PATH}")

        for path in (GOALS_MODEL, CORNERS_MODEL, CARDS_MODEL, SHOTS_MODEL):
            if not path.exists():
        ...
        self.shots_model = joblib.load(SHOTS_MODEL)
        ...
        self.shots_cols = model_columns(self.shots_model)
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            # Preferimos una tabla dedicada de próximos fixtures.
            # Así no mezclamos histórico Football-Data con calendario FotMob.
            has_upcoming = conn.execute(
                """
                SELECT 1
                FROM sqlite_master
                WHERE type='table'
                  AND name='laliga_upcoming_fixtures'
                """
            ).fetchone()
    ...
    def next_round(self):
        ...
        blocked = status.str.contains(
            "finished|full-time|full time|abandoned|cancelled|canceled|postponed",
            regex=True,
        )

        future = df[
            (df["utc_time"] >= now - pd.Timedelta(hours=6))
            & (~blocked)
        ].copy()
        ...
        # La tabla dedicada ya corresponde a la temporada actual,
        # pero conservamos la temporada más reciente por seguridad.
        latest_season = int(
            pd.to_numeric(
                future["season"],
                errors="coerce",
        ...
        if meaningful_round:
            ...
            if not same_round.empty:
                return same_round.sort_values(
                    ["utc_time", "match_id"]
                ).reset_index(drop=True)

        # Fallback si FotMob no trae ronda: cogemos el bloque de
        # partidos desde el primero hasta 5 días después.
        start_day = first["utc_time"].floor("D")
        end_day = start_day + pd.Timedelta(days=5)

        block = future[
            (future["utc_time"] >= start_day)
            & (future["utc_time"] < end_day)
        ].copy()

        if not block.empty:
            return block.reset_index(drop=True)

        # Último fallback: primeros 10 futuros.
        return future.head(10).reset_index(drop=True)
    ...
    def _build_standard_side_row(
        ...
        for col in cols:
            ...
            if expected_kind == "sot":
                m = re.fullmatch(
                    r"expected_sot_(last5|last10|season)",
                    col,
                )
                if m:
                    values = [
    ...
    def predict_goals(self, fixture):
        ...
        output["home_win_raw"] = ph
        output["draw_raw"] = pd_
        output["away_win_raw"] = pa
    ...
    def _sot_baseline_lambdas(self, cutoff):
        h = self._match_level_history(cutoff)
        ...
        home = pd.to_numeric(
            h["shots_on_target_for"],
            errors="coerce",
        ).fillna(0).to_numpy(dtype=float)

        away = pd.to_numeric(
            h["shots_on_target_against"],
            errors="coerce",
        ).fillna(0).to_numpy(dtype=float)

        return (
            float(np.average(home, weights=w)),
            float(np.average(away, weights=w)),
        )

    def predict_shots_on_target(self, fixture):
        cutoff = fixture["utc_time"]
        season = int(fixture["season"])
        ...
        lh = float(
            np.clip(
                self.shots_model.predict(home_row)[0],
                0.10,
                12.0,
            )
        )
        ...
        la = float(
            np.clip(
                self.shots_model.predict(away_row)[0],
                0.10,
                12.0,
            )
        )

        bh, ba = self._sot_baseline_lambdas(cutoff)
        ...
        for line in (7.5, 8.5, 9.5, 10.5, 11.5):
            market = f"total_over_{str(line).replace('.', '_')}"

            raw = poisson_over(line, total_model)
            base = poisson_over(line, total_base)

            weight = self._latest_shrink_weight(
                "shots_on_target_v1_backtest_predictions",
                market,
                total_fallbacks[line],
            )
        ...
        for line in (2.5, 3.5, 4.5, 5.5, 6.5):
            market = f"team_over_{str(line).replace('.', '_')}"
            ...
            raw_home = poisson_over(line, lh)
            base_home = poisson_over(line, bh)

            raw_away = poisson_over(line, la)
            base_away = poisson_over(line, ba)

            key = str(line).replace(".", "_")

            output[f"home_over_{key}"] = float(
                np.clip(
                    weight * raw_home
                    + (1 - weight) * base_home,
                    0,
    ...
    def predict_fixture(self, fixture):
        ...
        shots_on_target = self.predict_shots_on_target(fixture)
