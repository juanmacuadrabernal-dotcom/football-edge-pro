...
from football_odds_oddspapi_v15_1_4 import populate_match_odds, get_api_key, DEFAULT_BOOKMAKERS
...
def market_page(match,league,cat,min_ev):
    ...
    if cat=="sot":
        st.info("🎯 SOT: ahora buscamos automáticamente mercados de tiros a puerta TOTAL y por EQUIPO. Los props de jugadores se excluyen para no mezclar mercados distintos.")
    ...
    if cat=="sot" and auto_status.get("ok"):
        candidates = auto_status.get("sot_candidates", []) or []
        auto_rows = int(auto_status.get("sot_auto_rows", 0) or 0)
        if auto_rows:
            st.success(f"🎯 SOT automático detectado en {auto_rows} líneas del modelo.")
        elif candidates:
            st.warning("OddsPapi devuelve mercados de SOT para este partido, pero son props de jugador o no coinciden con nuestras líneas total/equipo.")
        else:
            st.caption("OddsPapi no publica todavía un mercado SOT total/equipo compatible para este partido.")
        with st.expander("🧪 Ver mercados SOT detectados por OddsPapi"):
            if candidates:
                st.dataframe(pd.DataFrame(candidates), hide_index=True, use_container_width=True)
            else:
                st.write("No se detectaron mercados SOT en Bet365 / Winamax / Pinnacle para este fixture.")
