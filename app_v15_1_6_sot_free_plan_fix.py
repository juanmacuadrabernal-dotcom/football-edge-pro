...
from football_odds_hybrid_v15_1_6 import populate_match_odds, get_api_key, DEFAULT_BOOKMAKERS
...
def market_page(match,league,cat,min_ev):
    ...
    if cat=="sot":
        st.info("🎯 SOT: Bet365/Pinnacle se cargan con SportsGameOdds usando las líneas TOTAL, LOCAL y VISITANTE exactas. Los props de jugadores quedan excluidos.")
    ...
    if cat=="sot":
        if auto_status.get("sot_ok"):
            st.success("🎯 SOT automático conectado · SportsGameOdds · LA_LIGA · Bet365 / Pinnacle")
            if auto_status.get("sot_note"):
                st.caption(str(auto_status.get("sot_note")))
        else:
            st.warning("🎯 SOT automático aún no disponible: " + str(auto_status.get("sot_message", "sin datos")))
            st.caption(str(auto_status.get("sot_note", "")))
