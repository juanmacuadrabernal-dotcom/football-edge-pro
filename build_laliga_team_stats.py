...
def season_diagnostic(conn: sqlite3.Connection) -> pd.DataFrame:
        ...
        SELECT
            ...
            SUM(
                CASE
                    ...
                    THEN 1 ELSE 0
                END
            ) AS eligible_for_model
        FROM fotmob_matches
        WHERE CAST(season AS INTEGER) >= 2023
        GROUP BY CAST(season AS INTEGER)
...
def validate_latest_season_included(
    conn: sqlite3.Connection,
    df: pd.DataFrame,
) -> None:
    diag = season_diagnostic(conn)

    if diag.empty:
        raise RuntimeError(
            "No hay temporadas >= 2023 en fotmob_matches."
        )

    print()
    print("Diagnóstico por temporada:")
    print(
        diag.to_string(index=False)
    )

    latest = int(diag["season"].max())
    ...
    print()
    print(
        f"Temporada actual {latest}/{latest+1}: "
        f"elegibles={expected} · cargados={included}"
    )

    if expected > 0 and included != expected:
        raise RuntimeError(
            "ERROR DE SINCRONIZACIÓN: la temporada actual no se "
            "ha cargado completa en team features. "
            f"Esperados={expected}, cargados={included}. "
            "Se aborta para NO reentrenar modelos incompletos."
        )

    if expected > 0 and included == 0:
        raise RuntimeError(
            "La temporada actual tiene partidos terminados pero "
            "ninguno entró en el dataset de modelos."
        )
...
def load_matches(conn: sqlite3.Connection) -> pd.DataFrame:
    ...
    LEFT JOIN (
        SELECT
            match_id,
            MAX(utc_time) AS referee_utc_time
        FROM referee_match_history
        GROUP BY match_id
    ) r
        ON r.match_id = m.match_id

    WHERE COALESCE(m.finished, 0) = 1
      AND CAST(m.season AS INTEGER) >= 2023
    ...
    # Parseo robusto de hora. En la migración 2026/27 algunos
    # valores de fotmob_matches.utc_time pueden venir en un formato que
    # Pandas no interpreta correctamente. referee_match_history comparte
    # los IDs FotMob reales y conserva el kickoff exacto.
    ...
    referee_ts = pd.to_datetime(
        df["referee_utc_time"],
        utc=True,
        errors="coerce",
    )

    df["utc_time"] = match_ts.where(
        match_ts.notna(),
        referee_ts,
    )

    used_ref_fallback = int(
        (match_ts.isna() & referee_ts.notna()).sum()
    )

    unresolved = df["utc_time"].isna()

    if used_ref_fallback:
        print(
            f"    Kickoffs recuperados desde historial de árbitros: "
            f"{used_ref_fallback}"
        )
    ...
    if unresolved.any():
        ...
        print()
        print("ERROR: partidos sin kickoff UTC recuperable:")
        print(bad.to_string(index=False))

        raise RuntimeError(
            f"Hay {int(unresolved.sum())} partidos sin hora UTC válida. "
            "Se aborta para evitar leakage o desorden cronológico."
        )

    # Ya tenemos un datetime UTC exacto para todas las filas.
    df = df.copy()

    validate_latest_season_included(
        conn,
        df,
    )
...
def main():
    print()
    print("=" * 72)
    print("LALIGA EDGE PRO - CONSTRUIR ESTADISTICAS DE EQUIPOS")
    print("=" * 72)
