...
try:
    from prediction_engine_laliga_v3 import PredictionEngineLaLiga
except Exception:
...
def run_laliga_updater():
    updater = BASE_DIR / "update_laliga_fixtures_v3.py"

    if not updater.exists():
        return False, (
            "Falta update_laliga_fixtures_v2.py "
...
with st.sidebar:
    ...
    if league_key == "laliga":
        if st.button(
            ...
            with st.spinner(
                "Descargando próximos partidos de LaLiga..."
            ):
                ok, output = run_laliga_updater()

            if ok:
                st.session_state["laliga_update_message"] = (
                    "✅ Jornada de LaLiga actualizada."
                )
...
st.caption("Football Edge Pro · Multiliga v8 · fixtures dedicados")
st.caption(
...
if not predictions:
    if league_key == "laliga":
        st.warning(
            ...
            "Pulsa el botón de abajo: la v8 la guarda en una "
            "tabla exclusiva y verifica el resultado antes de recargar."
        )
        ...
        if st.button(
            ...
            with st.spinner(
                "Buscando partidos en FotMob y guardándolos..."
            ):
                ok, output = run_laliga_updater()
...
if league_key == "eliteserien":
    ...
    MARKETS = [
        ...
        ("cards", "over_2_5", "🟨 Over 2.5 amarillas", "APROBADO"),
        ("cards", "over_3_5", "🟨 Over 3.5 amarillas", "FUERTE"),
        ("cards", "over_4_5", "🟨 Over 4.5 amarillas", "APROBADO"),
        ("cards", "over_5_5", "🟨 Over 5.5 amarillas", "SECUNDARIO"),
    ]
