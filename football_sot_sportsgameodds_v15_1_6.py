...
# SportsGameOdds free tier requires leagueID or eventID on /events.
LEAGUE_IDS = {
    "laliga": "LA_LIGA",
}
...
def fetch_sot_events(league_id: str, odd_ids: tuple[str, ...]) -> dict[str, Any]:
    if not odd_ids:
...
def populate_sot_odds(
    ...
    league_id = LEAGUE_IDS.get(str(league_key))
    if not league_id:
        return {
            "ok": False,
            "message": (
                "SportsGameOdds Free exige leagueID y esta liga no está "
                "disponible en el mapeo gratuito actual."
            ),
            "prices": {},
        }
    ...
    fetched = fetch_sot_events(league_id, tuple(odd_ids))
    ...
    return {
        ...
        "league_id": league_id,
        "sot_rows_with_price": len(prices),
        "note": (
