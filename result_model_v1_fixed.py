...
def load_v4_predictions(conn):
    ...
    # Hay una fila por mercado en V4. Como filtramos over_25,
    # debería quedar exactamente una por partido.
...
def calibrate_walkforward(raw):
    ...
    for season in seasons:
        ...
        # En la primera temporada OOS no tenemos predicciones
        # OOS anteriores suficientes para calibrar sin leakage.
        if len(prior) < 200:
            raw.loc[
                mask,
                [
        ...
        # Compatibilidad con scikit-learn moderno:
        # multi_class fue eliminado. Con lbfgs y >2 clases
        # se usa multinomial automáticamente.
        ...
        for j, cls in enumerate(CLASSES):
            if cls not in class_to_idx:
                raise RuntimeError(
                    f"Clase {cls} no encontrada "
                    f"en calibrador: {model.classes_}"
                )
...
def main():
    ...
    print("=" * 76)
    print(
        "ELITESERIEN EDGE PRO - RESULT MODEL V1 FIXED"
    )
    print("=" * 76)
    print(
    ...
    with sqlite3.connect(
        ...
        print(
            "1/5 Cargando predicciones V4..."
        )
        pred = load_v4_predictions(
            conn
        )

        print(
            "2/5 Cargando histórico de resultados..."
        )
        hist = load_all_historical_results(
            conn
        )

        print(
            "3/5 Calculando probabilidades 1X2..."
        )
        ...
        print(
            "4/5 Calibrando walk-forward..."
        )
        final = calibrate_walkforward(
            raw
        )

        print(
            "5/5 Evaluando y guardando..."
        )
        summary = summarize(
            final
        )
    ...
    for r in summary.itertuples(
        ...
        print(
            f"  Accuracy: "
            f"modelo={r.accuracy_final*100:5.1f}% | "
    ...
    print("=" * 76)
    print(
        "Si Brier y LogLoss mejoran el baseline "
        "de forma consistente, 1X2 pasa a cuotas + EV."
    )
    print("=" * 76)
